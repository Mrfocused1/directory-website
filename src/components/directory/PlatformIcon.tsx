...
const ICONS: Record<Platform, { svg: React.ReactNode; color: string; label: string }> = {
  instagram: {
    label: "Instagram",
    color: "text-pink-600",
    svg: (
...
export default function PlatformIcon({
  ...
  const { svg, color, label } = ICONS[platform];
  const sizeClass = size === "xs" ? "w-4 h-4" : size === "md" ? "w-6 h-6" : "w-5 h-5";

  return (
    <span className={`inline-flex items-center gap-1 ${color}`} title={label}>
      <span className={`${sizeClass} flex items-center justify-center`}>{svg}</span>
